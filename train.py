...
class VideoDescriptionTrain(object):
    ...
    def load_dataset(self, training_list):
        ...
        train_sequences = np.array(train_sequences, dtype=object)  # Ensure it's a NumPy array
        ...
        for i in range(self.epochs):
            for idx in range(0, file_size):
                ...
                y = to_categorical(train_sequences[idx], self.num_decoder_tokens)  # Ensure dtype is correct
                decoder_input_data.append(y[:-1])
                decoder_target_data.append(y[1:])

                if n == self.batch_size:
                    encoder_input = np.array(encoder_input_data, dtype='float32')
                    decoder_input = np.array(decoder_input_data, dtype='float32')
                    decoder_target = np.array(decoder_target_data, dtype='float32')
                    ...
                    yield (encoder_input, decoder_input), decoder_target  # Return as tuple instead of list
